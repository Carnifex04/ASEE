...
def calculate_s1_returns(df, initial_capital, share_size):
    df["Position"] = share_size * \
        np.where(df["Entry/Exit_S1"] > 0.0, 1,
                 (np.where(df["Entry/Exit_S1"] < 0.0, -1, 0)))
    ...
    df["Portfolio Cumulative Returns"] = (
        1 + df["Portfolio_Daily_Returns"]).cumprod() - 1
    return df
...
def calculate_s2_returns(df, initial_capital, share_size):
    df["Position"] = share_size * \
        np.where(df["Entry/Exit_S2"] > 0.0, 1,
                 (np.where(df["Entry/Exit_S2"] < 0.0, -1, 0)))
